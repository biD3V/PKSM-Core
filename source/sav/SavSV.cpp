...
namespace pksm
{
    SavSV::SavSV(const std::shared_ptr<u8[]>& dt, size_t length) : Sav9(dt, length)
    {
        game = Game::SV;

        Box            = 0x0d66012c;
        WondercardData = 0x99E1625E;
        Party          = 0x3AA1A9AD;
        PokeDex        = 0x0DEAAEBD;
        Items          = 0x21C9BD44;
    ...
    {
        return LittleEndian::convertTo<u32>(getBlock(Money)->decryptedData());
    }

    void SavSV::money(u32 v)
    {
        LittleEndian::convertFrom<u32>(getBlock(Money)->decryptedData(), v);
    }
    ...
    {
        return getBlock(Money)->decryptedData()[0x11C];
    }
    ...
    std::vector<std::pair<Sav::Pouch, int>> SavSV::pouches(void) const
    {
        return {
            {Pouch::Medicine,   60 },
            {Pouch::Ball,       30 },
    ...
    std::map<Sav::Pouch, std::vector<int>> SavSV::validItems(void) const
    {
        return {
            {Pouch::Medicine,
    ...
    std::unique_ptr<PKX> SavSV::pkm(u8 slot) const
    {
        ...
        return PKX::getPKM<Generation::NINE>(
            getBlock(Party)->decryptedData() + offset, PK9::PARTY_LENGTH);
    }
    ...
    {
        if (pk.generation() == Generation::NINE)
        {
    ...
    void SavSV::cryptBoxData(bool crypted)
    {
        for (u8 box = 0; box < maxBoxes(); box++)
        {
            ...
            {
                std::unique_ptr<PKX> pk8 = PKX::getPKM<Generation::NINE>(
    ...
    {
        if (wc.generation() == Generation::NINE)
        {
            ...
            if (wc8.pokemon())
            {
                ...
                auto pk8 = PKX::getPKM<Generation::NINE>(nullptr, PK9::BOX_LENGTH);
    ...
    std::unique_ptr<WCX> SavSV::mysteryGift(int) const
    {
        return nullptr;
    }
    ...
    void SavSV::dex(const PKX& pk)
    {
        ...
        if (!pk.egg())
        {
            if (u16 index = ((PK9&)pk).pokedexIndex())
            {
        ...
        if (entryAddr)
        {
            ...
            {
                form *= 7;
                form += static_cast<const PK9&>(pk).formDuration();
            }
